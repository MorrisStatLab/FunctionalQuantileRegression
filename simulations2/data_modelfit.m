...
seed = randi([0 10000],2,100);
seed2 = randi([0 10000],2,100);
...
for i1=1:100
    load(sprintf('Y:/submission/simulations2/data/model%d.mat',i1))
    for i2=1:2
        result=FQR_HS(model,qt(i2),MCMCspecs,seed(i2,i1));
        MCMC_betat=result.MCMC_betat;
        MCMC_P=MCMC_betat(:,(model.T+1):end);
        save(sprintf('Y:/submission/simulations2/output/HS/MCMC_betat_%d_rep_%d.mat',qt(i2)*100,i1),'MCMC_P');
    end
    clear model;
end
...
for i1=1:100
    ...
    for i2=1:2
        ...
        MCMC_betat=result.MCMC_betat;
        MCMC_P=MCMC_betat(:,(model.T+1):end);
        save(sprintf('Y:/submission/simulations2/output/QR/MCMC_betat_%d_rep_%d.mat',qt(i2)*100,i1),'MCMC_P');
    end
    clear model;
end
...
% Please refer to the R code submission/simulations2/data_modelfit_bootstrap.R
...
for i1=1:100
    for i2=1:2
        raw=dlmread(sprintf('Y:/submission/simulations2/output/freqQR/raw/BS_betat_%d_rep_%d.txt',qt(i2)*100,i1));
        smoothed=NaN(size(raw));
        for i=1:size(raw,1)
            smoothed(i,:)=wden(raw(i,:),'heursure','s','mln',4,'db4');
        end
        dlmwrite(sprintf('Y:/submission/simulations2/output/freqQR/wavelets/BS_betat_%d_rep_%d.txt',qt(i2)*100,i1),smoothed,'delimiter','\t','precision','%12.6e');
        clear smoothed;
    end
end
