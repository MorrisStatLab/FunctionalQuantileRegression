...
for i1=1:100
    ...
    for i2=1:2
        ...
        MCMC_betat=result.MCMC_betat;
        MCMC_P=MCMC_betat(:,(model.T+1):end);
        save(sprintf('./output/QR/MCMC_betat_%d_rep_%d.mat',qt(i2)*100,i1),'MCMC_P');
    end
    clear model;
end
...
% Please refer to the R code simulations2/data_modelfit_bootstrap.R
...
for i1=1:100
    for i2=1:2
        raw=dlmread(sprintf('./output/freqQR/raw/BS_betat_%d_rep_%d.txt',qt(i2)*100,i1));
        smoothed=NaN(size(raw));
        for i=1:size(raw,1)
            smoothed(i,:)=wden(raw(i,:),'heursure','s','mln',4,'db4');
        end
        dlmwrite(sprintf('./output/freqQR/wavelets/BS_betat_%d_rep_%d.txt',qt(i2)*100,i1),smoothed,'delimiter','\t','precision','%12.6e');
        clear smoothed;
    end
end
